...
enum Commands {
    ...
    Create {
        ...
        help = "ORM to use",
        value_name = "ORM",
        value_parser = EnumValueParser::< BackendOrm >::new(),
        ignore_case = true,
        )]
        backendorm: Option<BackendOrm>,
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Some(command) => {
            ...
            match command {
                Commands::Create {
                    ...
                    backendframework,
                    plugins,
                } => create_project(cli_mode, name, database, backendorm, backendframework, plugins)?,
                Commands::Configure {
                    query_sync,
                    qsync_input_files,
                    qsync_output_file,
        ...
        None => {
            ...
            match cli.name {
                Some(name) => create_project(false, name, None, None, None, None)?,
...
fn create_project(
    ...
    let backend_orm = match orm {
        Some(orm) => orm,
        None => {
            if cli_mode {
                panic!("Fatal: No ORM specified")
            }
            logger::message("Select a database ORM to use:");
            logger::message("Use UP/DOWN arrows to navigate and SPACE or ENTER to confirm.");
            let items = vec!["diesel", "prisma"];
            let selection = Select::with_theme(&ColorfulTheme::default())
                .items(&items)
                .default(0)
                .interact_on_opt(&Term::stderr())?;
            ...
            match selection {
                Some(0) => BackendOrm::Diesel,
                Some(1) => BackendOrm::Prisma,
                _ => panic!("Fatal: Unknown backend ORM specified."),
            }
        }
    };
    ...
    cra_enabled_features.push( match backend_orm {
        BackendOrm::Prisma => {
            if backend_database == BackendDatabase::Postgres {
                "orm_prisma_psql".to_string()
            } else {
                "orm_prism_sqlite".to_string()
            }
        }
        BackendOrm::Diesel => {
            if backend_database == BackendDatabase::Postgres {
                "orm_diesel_psql".to_string()
            } else {
                "orm_diesel_sqlite".to_string()
            }
        }
    });
